...
import org.rage.logger.LoggerHelper;
...
public class TransformationHelper {
	 ...
	 *
	 */
	public static <T> List<Map<String, Map<String, Object>>> transformListToListMap(final List<T> instanceList) {
		final List<Map<String, Map<String, Object>>> mapList = new ArrayList<>(instanceList.size());
	 ...
	 *         format
	 */
	public static Map<String, Map<String, Object>> transformObjectToMap(final Object instance) {
	 ...
	 */
	private static void transformObject(final Object instance, final Map<String, Boolean> excludeList,
		...
		for (final Field field : fields) {
			if (!excludeList.containsKey(field.getName())) {
				try {
					...
					final Object getterValue = getGetterMethod(field.getName(), instance).invoke(instance);
	 ...
	 * @param clazz
	 */
	public static <T> void populateObjectListWithProperties(final List<T> instances,
			final List<Map<String, Map<String, Object>>> objectList, final Class<T> clazz) {
	...
	public static void populateObjectWithProperties(final Object instance,
		...
		for (final String attribute : metadata.keySet()) {
			final Method setterMethod = getSetterMethod(attribute, instance.getClass());

			// We "suppose" that the null are not present in the attributes lists
			try {
				setValuesToAttribute(instance, setterMethod, metadata.get(attribute));
			} catch (final Exception e) {
				LoggerHelper.printException(e);
			}
		}
	 ...
	 */
	private static Method getGetterMethod(final String fieldName, final Object instance)
			throws NoSuchMethodException, SecurityException {
		final String getterName = "get" + fieldName.substring(0, 1).toUpperCase()
	...
	private static Method getSetterMethod(final String attributeName, final Class clazz) {
		Method setterMethod = null;
		final String setterName = "set" + attributeName.substring(0, 1).toUpperCase()
	...
	private static void setValuesToAttribute(final Object instance, final Method setterMethod,
			final Map<String, Object> value) throws Exception {
		if (value == null) {
			return;
		}
		...
		if (Date.class.getCanonicalName().equals(type) || java.sql.Date.class.getCanonicalName().equals(type)
			...
			setterMethod.invoke(instance, sfd.parse((String) value.get(MAP_VALUE_KEY)));

		} else if ((BigDecimal.class.getCanonicalName().equals(type))) {
			setterMethod.invoke(instance, new BigDecimal((String) value.get(MAP_VALUE_KEY)));

		} else if (type.startsWith(FITNESS_CLASS_PACKAGE)) {
			final Object innerObjectInstance = Class.forName(type).newInstance();
		...
		} else if (ArrayList.class.getCanonicalName().equals(type) || Set.class.getCanonicalName().equals(type)) {
			...
			final List<Map<String, Map<String, Object>>> listValues = (List<Map<String, Map<String, Object>>>) value
					.get(MAP_VALUE_KEY);
			final Iterator<Map<String, Map<String, Object>>> it = listValues.iterator();
			...
			setterMethod.invoke(instance, Integer.parseInt((String)value.get(MAP_VALUE_KEY)));
		}else if(Long.class.getCanonicalName().equals(type)){
			setterMethod.invoke(instance, Long.parseLong((String)value.get(MAP_VALUE_KEY)));
		}else{
	...
	private static Map<String, Object> giveMeTheMap(final Object value) {
		...
		} else if (value.getClass().getPackage().getName().contains(FITNESS_CLASS_PACKAGE)) {
		...
		classType = classType != null ? classType : value.getClass().getCanonicalName();
